...
/*lint -e124 Pointer to void not allowed but commonly used in expression in va_arg  */
...
#endif
#include <ctype.h>
#if defined(USE_KERNEL)		/* Used by tests/benchmark/coremark */
#include "Os.h"
#endif
 ...
 */
static inline int emitChar( FILE *file, char **buf, char c, int *left ) {
    (void)file; //lint !e920 MISRA False positive. Allowed to cast pointer to void here.
...
width:
            ...
            if( (ch >= '0')  && (ch <= '9') ) {
                ...
                wBuff[a] = '\0';
                 width = (int)strtoul(wBuff,NULL,10);
    ...
    int ret = (n-left);
//	va_end(ap);		// Removed, IMPROVMENT: Check the va_start/va_end handling (used in calling functions also).
    if(buffer!=0) {
        left = 0;
