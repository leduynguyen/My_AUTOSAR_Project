 ...
 * @return void
 */
static void initPhy(uint8 CtrlIdx){
...
/**
 * Master or slave configuration for the PHY
 * @param CtrlIdx
 * @param MasterSlaveBit, 1 is master and 0 is slave
 * @return void
 */
static void setMasterSlave(uint8 CtrlIdx, uint8 MasterSlaveBit){
	uint16 regData = 0;

	//7.0202.12 is MasterSlaveBit <- Force Master
	(void) readMii45(CtrlIdx, PHY_MMD_AUTONEG, PHY_AUTONEG_ADV_REG1, &regData);
	regData = (regData & 0xefff)|(MasterSlaveBit<<12);
	(void) writeMii45(CtrlIdx, PHY_MMD_AUTONEG, PHY_AUTONEG_ADV_REG1,regData);

	//1.0834.14 is masterSlave Config
	(void) readMii45(CtrlIdx, PHY_MMD_PMA_PMD, PHY_PMA_PMD_100BASET1_CONTROL, &regData);
	regData = (regData & 0xbfff)|(MasterSlaveBit<<14);
	(void) writeMii45(CtrlIdx, PHY_MMD_PMA_PMD, PHY_PMA_PMD_100BASET1_CONTROL,regData);

	//follow by software reset 7.0200.15 0r 1.0.15?
	softReset(CtrlIdx,PHY_MMD_PMA_PMD);
}
...
/**
 * Power down the transceiver or bring it to normal operation
 * @param ctrlIdx
 * @param bPowerDown
 * @return none
 */
static void powerDown(uint8 ctrlIdx,boolean bPowerDown){
	uint16 regValue = 0;
	(void)readMii45(ctrlIdx,PHY_MMD_PCS,PHY_PCS_COPPER_CONTROL, &regValue);
	if(bPowerDown == FALSE){
		regValue &= ~PHY_POWER_DOWN;
	}else{
		regValue |= PHY_POWER_DOWN;
	}
	(void)writeMii45(ctrlIdx,PHY_MMD_PCS,PHY_PCS_COPPER_CONTROL, regValue);
}
...
#if defined(CFG_ETH_PHY_DP83848C) || defined(CFG_ETH_PHY_DP83865) || defined(CFG_ETH_PHY_TJA1100) || defined(CFG_ETH_PHY_88Q1010)
    uint16 phyStatus;
    uint16 timeout;
