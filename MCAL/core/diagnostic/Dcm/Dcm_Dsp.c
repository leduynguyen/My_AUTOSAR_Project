...
static Dcm_NegativeResponseCodeType udsReadDtcInfoSub_0x06_0x10(const PduInfoType *pduRxData, PduInfoType *pduTxData)
{
    ...
    if (responseCode == DCM_E_POSITIVERESPONSE) {
        ...
        /* @req DCM295 */ 
        /* @req DCM475 */
        getStatusOfDtcResult = Dem_GetStatusOfDTC(dtc, dtcOrigin, &statusOfDtc);
                    ...
                    for( uint16 i = 0; i < (didPtr->DspNofSignals) && (DCM_E_POSITIVERESPONSE == responseCode); i++ ) {
                        ...
                        if(signalPtr->DspSignalDataRef->DspDataUsePort == DATA_PORT_BLOCK_ID) {
                            ...
                            responseCode = writeDataToNvMBlock(signalPtr->DspSignalDataRef->DspNvmUseBlockID,
                                                                &pduRxData->SduDataPtr[3 + signalPtr->DspSignalsPosition],
                            ...
                            if( NULL != signalPtr->DspSignalDataRef->DspDataWriteDataFnc.FixLenDataWriteFnc ) {
                                if(signalPtr->DspSignalDataRef->DspDataInfoRef->DspDidFixedLength) { /* @req DCM794 */
                                    /* @req DCM395 */
                                    result = signalPtr->DspSignalDataRef->DspDataWriteDataFnc.FixLenDataWriteFnc(&pduRxData->SduDataPtr[3 + signalPtr->DspSignalsPosition], opStatus, &responseCode);
                                } else {
                                    /* @req DCM395 */
                                    result = signalPtr->DspSignalDataRef->DspDataWriteDataFnc.DynLenDataWriteFnc(&pduRxData->SduDataPtr[3 + signalPtr->DspSignalsPosition], dataLen, opStatus, &responseCode);
                                }
...
static void DspSamplePDids(uint32 period, PduIdType rxPduId) {
    for(uint8 i = 0; i < dspPDidRef.PDidNofUsed; i++) {
        if( (dspPDidRef.dspPDid[i].PDidTxPeriod == period) && (dspPDidRef.dspPDid[i].PDidRxPduID == rxPduId) ) {
            uint16 pdidLength = 0;
            if( DCM_E_POSITIVERESPONSE == getPDidData(TO_PERIODIC_DID(dspPDidRef.dspPDid[i].PeriodicDid), dspPDidRef.dspPDid[i].PdidData, &pdidLength) ) {
...
{
    Dcm_NegativeResponseCodeType responseCode = DCM_E_SERVICENOTSUPPORTED;
    //lint -estring(920,pointer)  /* cast to void */
    (void)pduRxData;
    (void)pduTxData;
    //lint +estring(920,pointer)  /* cast to void */
    ...
    //pduTxData->SduDataPtr[2] = (uint8)(128 + 2);
