 ...
 * Local types
 */
#if !defined(USE_NVM) && !defined(USE_RTE)
...
typedef struct {
    ...
    boolean dataModified; /* true if buffer changed */
} BufferInfo_t;
...
static BufferInfo_t IndicatorsAreModified;
#endif
#if (DEM_STORE_UDS_STATUS_BIT_SUBSET_FOR_ALL_EVENTS == STD_ON)
static BufferInfo_t StatusBitsAreModified;
#endif
#endif
...
static void setIndicatorBlockChanged(void);
#endif
...
static boolean checkDtcKind(Dem_DTCKindType dtcKind, const Dem_EventParameterType *eventParam)
{
    ...
    if( (NULL != eventParam->DTCClassRef) && (DTCIsAvailable(eventParam->DTCClassRef)==TRUE) ) {
        result = (dtcKind == DEM_DTC_KIND_ALL_DTCS) || (eventParam->DTCClassRef->DTCKind == dtcKind);
    }
...
static boolean checkEntryValid(Dem_EventIdType eventId, Dem_DTCOriginType origin){
    ...
    if (NULL != EventIdParam) {
        ...
        isValid = checkDtcOrigin(origin, EventIdParam) && (NULL != eventStatusRec) && eventStatusRec->isAvailable;
    } else {
 ...
 */
/* @req 4.2.2/DEM_00915 */
static boolean lookupEventOfUdsDTC(uint32 dtc, EventStatusRecType **eventStatusRec)
{
...
static boolean matchEventWithDtcFilter(const EventStatusRecType *eventRec)
{
    ...
    // Check status
    if ((dtcFilter.dtcStatusMask == DEM_DTC_STATUS_MASK_ALL) || (0!= (eventRec->eventStatusExtended & dtcFilter.dtcStatusMask))) {
            ...
            if ((storeIndex + recordSize + DEM_PID_IDENTIFIER_SIZE_OF_BYTES) <= DEM_MAX_SIZE_FF_DATA) {
                ...
                if(FFIdClassRef[i]->PidReadFnc != NULL){
                    ...
                    if (callbackReturnCode != E_OK) {
                        ...
                        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GET_FREEZEFRAME_ID, DEM_E_NODATAAVAILABLE);
                    }
                                                ...
                                                boolean* eventHandled, Dem_DTCOriginType origin, boolean removeOldFFRecords)
{
    ...
    if( eventParam->MaxNumberFreezeFrameRecords == nofStoredMemory ) {
        ...
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GLOBAL_ID, DEM_E_MEMORY_CORRUPT);
...
#if (DEM_STORE_UDS_STATUS_BIT_SUBSET_FOR_ALL_EVENTS == STD_ON)
/**
 * Sets UDS status bit subset block as changed
 */
static void setStatusBitBlockChanged(void) {
    if( E_OK == setNvMBlockChanged(StatusBitsAreModified.blockId) ) {
        StatusBitsAreModified.dataModified = FALSE;
    } else {
        StatusBitsAreModified.dataModified = TRUE;
    }
}
#endif
...
#if defined(DEM_USE_INDICATORS)
static void setIndicatorBlockChanged(void) {
    if( E_OK == setNvMBlockChanged(IndicatorsAreModified.blockId) ) {
        IndicatorsAreModified.dataModified = FALSE;
    } else {
        IndicatorsAreModified.dataModified = TRUE;
    }
}
#endif
static void setEventBlockChanged(Dem_DTCOriginType origin)
...
static Std_ReturnType storeEventEvtMem(const Dem_EventParameterType *eventParam, const EventStatusRecType *eventStatus)
{
    ...
    switch (eventParam->EventClass->EventDestination) {
        ...
        case DEM_DTC_ORIGIN_SECONDARY_MEMORY:
#if (DEM_USE_SECONDARY_MEMORY_SUPPORT == STD_ON)
            ret = storeEventMem(eventParam, eventStatus, secMemEventBuffer, DEM_MAX_NUMBER_EVENT_ENTRY_SEC, DEM_DTC_ORIGIN_SECONDARY_MEMORY);    /** @req DEM548 */
#endif
            break;
        case DEM_DTC_ORIGIN_PERMANENT_MEMORY:
        case DEM_DTC_ORIGIN_MIRROR_MEMORY:
...
static boolean checkOBDFFStorageCondition(const Dem_EventParameterType *eventParam, Dem_EventStatusExtendedType oldStatus, Dem_EventStatusExtendedType status)
{
    ...
    if ( (eventParam != NULL) && (NULL != eventStatusRec) && eventStatusRec->isAvailable ) {
        if ( operationCycleIsStarted(eventParam->EventClass->OperationCycleRef) ) {/* @req DEM481 */
            ...
            if ( eventProcessingAllowed(eventParam))  {
                ...
                if ( (0 != eventStatusRec->errorStatusChanged) || (0 != eventStatusRec->extensionDataChanged) ) {
                    eventStoreStatus = storeEventEvtMem(eventParam, eventStatusRec); /** @req DEM184 *//** @req DEM396 */
                    boolean storeExtData;
                    boolean storeFFData;
                    boolean overrideOldExtData;
                    ...
                    if( FALSE == eventDTCRecordDataUpdateDisabled(eventParam) ) {
                        ...
                        if (storeFFData) {
                            ...
                            if( (NULL != eventParam->DTCClassRef) && (DEM_DTC_KIND_EMISSION_REL_DTCS == eventParam->DTCClassRef->DTCKind) &&
                                    eventDataUpdated && checkOBDFFStorageCondition(eventParam, oldStatus, eventStatusRec->eventStatusExtended) ) {
...
static boolean ageEvent(EventStatusRecType* evtStatusRecPtr) {
    ...
    if( updatedMemory ) {
        if( !eventDeleted ) {
            if(E_OK == storeEventEvtMem(evtStatusRecPtr->eventParamRef, evtStatusRecPtr) ) {
                /* @req DEM475 */
            ...
            notifyEventDataChanged(evtStatusRecPtr->eventParamRef);
            setEventBlockChanged(evtStatusRecPtr->eventParamRef->EventClass->EventDestination);
        }
...
static Std_ReturnType handleAging(Dem_OperationCycleIdType operationCycleId)
{
    ...
    if( agingUpdatedSecondaryMemory ) {
        setEventBlockChanged(DEM_DTC_ORIGIN_SECONDARY_MEMORY);
    }
...
static boolean getFFRecData(const Dem_EventParameterType *eventParam, uint8 recNum, uint8 **freezeFrameData, uint8 *ffRecNumFound, Dem_FreezeFrameKindType *ffKind)
{
    ...
    if( (NULL != eventParam->FreezeFrameClassRef) && (NULL != eventParam->FreezeFrameRecNumClassRef) ) {
        ...
        if( (NULL != configSet->GlobalOBDFreezeFrameClassRef) && (NULL != eventParam->DTCClassRef) && (DEM_DTC_KIND_EMISSION_REL_DTCS == eventParam->DTCClassRef->DTCKind)) {
...
#if defined(DEM_USE_INDICATORS)
    IndicatorsAreModified.blockId = DEM_INDICATOR_NVM_BLOCK_HANDLE;
    IndicatorsAreModified.dataModified = FALSE;
#endif

#if (DEM_STORE_UDS_STATUS_BIT_SUBSET_FOR_ALL_EVENTS == STD_ON)
    StatusBitsAreModified.blockId = DEM_STATUSBIT_NVM_BLOCK_HANDLE;
    StatusBitsAreModified.dataModified = FALSE;
#endif
}
...
static void validateNvMBlockSizes(void)
{
    ...
    DEM_ASSERT( (0 == DEM_INDICATOR_NVM_BLOCK_HANDLE ) ||
                (DEM_INDICATOR_NVM_BLOCK_SIZE == sizeof(indicatorBuffer)));
#endif
}
#endif
...
void Dem_MainFunction(void)/** @req DEM125 */
{
    ...
    for (uint32 i=0;i<DEM_MAX_NR_OF_EVENT_DESTINATION;i++) {
        if( EventIsModified[i].dataModified==TRUE ) {
            setEventBlockChanged(EventIsModified[i].origin);
        }

        if( ExtendedDataIsModified[i].dataModified==TRUE) {
            setExtendedDataBlockChanged(ExtendedDataIsModified[i].origin);
        }

        if ( FFIsModified[i].dataModified ==TRUE) {
            setFreezeFrameBlockChanged(FFIsModified[i].origin);
        }

    }
#if defined(DEM_USE_INDICATORS)
    if(IndicatorsAreModified.dataModified) {
        setIndicatorBlockChanged();
    }
#endif
#if (DEM_STORE_UDS_STATUS_BIT_SUBSET_FOR_ALL_EVENTS == STD_ON)
    if(StatusBitsAreModified.dataModified==TRUE) {
        setStatusBitBlockChanged();
    }
#endif
#endif /* DEM_USE_MEMORY_FUNCTIONS */
...
#else
    (void)IndicatorId;
    DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GETINDICATORSTATUS_ID, DEM_E_PARAM_CONFIG);
    return E_NOT_OK;
#endif
}
...
Std_ReturnType Dem_SetOperationCycleState(Dem_OperationCycleIdType operationCycleId, Dem_OperationCycleStateType cycleState)
{
    ...
    if( DEM_ACTIVE == operationCycleId ) {
        /* Handled internally */
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_SETOPERATIONCYCLESTATE_ID, DEM_E_UNEXPECTED_EXECUTION);
        returnCode = E_NOT_OK;
    } else {
...
Dem_ReturnGetStatusOfDTCType Dem_GetStatusOfDTC(uint32 dtc, Dem_DTCOriginType dtcOrigin, Dem_EventStatusExtendedType* status) {
    ...
    if (lookupEventOfUdsDTC(dtc, &eventRec)==TRUE) {
        /* Event found for this DTC */
        if (checkDtcOrigin(dtcOrigin,eventRec->eventParamRef)==TRUE) {
            ...
            if (checkDtcOrigin(dtcOrigin, eventRec->eventParamRef)==TRUE) {
                if (lookupExtendedDataRecNumParam(extendedDataNumber, eventRec->eventParamRef, &extendedDataRecordClass, &posInExtData)==TRUE) {
                    if (*bufSize >= extendedDataRecordClass->DataSize) {
                        ...
                        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GETEXTENDEDDATARECORDBYDTC_ID, DEM_E_PARAM_LENGTH);
                    ...
                    if (*bufSize >= FFDataSize) {
                        switch (dtcOrigin) {
                            ...
                            case DEM_DTC_ORIGIN_MIRROR_MEMORY:
                                ...
                                DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GETFREEZEFRAMEDATABYDTC_ID, DEM_E_NOT_IMPLEMENTED_YET);
                                break;
                            default:
                                returnCode = DEM_GET_FFDATABYDTC_WRONG_DTCORIGIN;
                                break;
                        ...
                        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GETFREEZEFRAMEDATABYDTC_ID, DEM_E_PARAM_LENGTH);
...
Dem_ReturnDisableDTCRecordUpdateType Dem_DisableDTCRecordUpdate(uint32 DTC, Dem_DTCOriginType DTCOrigin)
{
    ...
    if(NO_DTC_DISABLED == DTCRecordDisabled.DTC) {
        ...
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_DISABLEDTCRECORDUPDATE_ID, DEM_E_WRONG_CONDITION);
...
Std_ReturnType Dem_EnableDTCRecordUpdate(void)
{
    ...
    if(NO_DTC_DISABLED != DTCRecordDisabled.DTC) {
        ...
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_ENABLEDTCRECORDUPDATE_ID, DEM_E_SEQUENCE_ERROR);
    }
        ...
        uint16 DataId, uint8* DestBuffer, uint8* BufSize, boolean CareAboutBufsize)
{
    ...
    if( DEM_INITIALIZED == demState  ) {
        ...
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GETEVENTFREEZEFRAMEDATA_ID, DEM_E_UNINIT);
    }
...
static Std_ReturnType Dem_GetEventExtendedDataRecord_Internal(Dem_EventIdType EventId, uint8 RecordNumber, uint8* DestBuffer, uint8* BufSize, boolean CareAboutBufsize)
{
    ...
    if( DEM_INITIALIZED == demState  ) {
        ...
        DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GETEVENTEXTENDEDDATARECORD_ID, DEM_E_UNINIT);
    }
