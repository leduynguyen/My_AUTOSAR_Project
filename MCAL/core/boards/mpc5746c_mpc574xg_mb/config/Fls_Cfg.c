...
const FlashType flashInfo[] = {
    ...
    so the bank size will be 0x01280000 - 0x00610000 even though available actual size is only 0x31C000 as computed above */
    [0].regBase = 0xFFFE0000UL, /* C55FMC_MCR register */

    /*HSM code*/
    [0].sectAddr[0] = 0x00610000,
    [0].addrSpace[0] = ADDR_SPACE_SET(ADDR_SPACE_LOW) + 2,
    [0].sectAddr[1] = 0x00620000,
    [0].addrSpace[1] = ADDR_SPACE_SET(ADDR_SPACE_LOW) + 3,
    ...
    [0].addrSpace[15] = ADDR_SPACE_SET(ADDR_SPACE_MID) + 0,
    [0].sectAddr[16] = 0x00FB8000,
    [0].addrSpace[16] = ADDR_SPACE_SET(ADDR_SPACE_MID) + 1,
    /*LOW*/
    [0].sectAddr[17] = 0x00FC0000,
    [0].addrSpace[17] = ADDR_SPACE_SET(ADDR_SPACE_LOW) + 0,
    [0].sectAddr[18] = 0x00FC8000,
    [0].addrSpace[18] = ADDR_SPACE_SET(ADDR_SPACE_LOW) + 1,
    [0].sectAddr[19] = 0x00FD0000,
    [0].addrSpace[19] = ADDR_SPACE_SET(ADDR_SPACE_LOW) + 2,
    [0].sectAddr[20] = 0x00FD8000,
    [0].addrSpace[20] = ADDR_SPACE_SET(ADDR_SPACE_LOW) + 3,
    [0].sectAddr[21] = 0x00FE0000,
    [0].addrSpace[21] = ADDR_SPACE_SET(ADDR_SPACE_LOW) + 0,
    [0].sectAddr[22] = 0x00FF0000,
    [0].addrSpace[22] = ADDR_SPACE_SET(ADDR_SPACE_LOW) + 1,

    /*Large Flash Blocks*/ /*LARGE*/
    [0].sectAddr[23] = 0x01000000,
