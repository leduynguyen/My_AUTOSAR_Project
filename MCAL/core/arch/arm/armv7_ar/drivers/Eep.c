...
typedef struct {
    ...
    Spi_DataBufferType ebE2Addr[ADDR_LENGTH];
...
void Eep_Init(const Eep_ConfigType* ConfigPtr) {
    ...
    (void)Spi_SetupEB(CFG_SPI_P()->EepAddrChannel, Eep_Global.ebE2Addr, NULL, sizeof(Eep_Global.ebE2Addr) / sizeof(Eep_Global.ebE2Addr[0]));
    ...
    do {
        ...
        case JOB_MAIN:
            if (job->left > 0) {
                ...
                convertToSpiAddr(Eep_Global.ebE2Addr, job->eepAddr);
