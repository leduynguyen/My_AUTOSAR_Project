...
ErrorStatus RCC_WaitForHSEStartUp(void)
{
  ...
  do
  {
    ...
    StartUpCounter++;  
  } while((StartUpCounter != HSE_STARTUP_TIMEOUT) && (HSEStatus ==(FlagStatus)RESET));
  
  if (RCC_GetFlagStatus(RCC_FLAG_HSERDY) != (FlagStatus)RESET)
  {
    status = SUCCESS;
  }
  else
...
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks)
{
  ...
  uint32_t tmp, pllmull = 0, pllsource = 0, presc;
...
FlagStatus RCC_GetFlagStatus(uint8_t RCC_FLAG)
{
  ...
  tmp =(uint32_t) RCC_FLAG & FLAG_Mask;
