...
void Fr_Init(const Fr_ConfigType* Fr_ConfigPtr) {
    ...
    for (n = 0; n < FR_ARC_CTRL_CONFIG_CNT; n++) {
        ...
        for (j = 0; j < Fr_Cfg.Fr_ConfigPtr->FrTrigConfig[n].FrNbrTrigConfiged; j++) {
            Fr_Cfg.Fr_ConfigPtr->FrTrigConfig[n].FrMsgBufferCfg[j].FrDataPartitionAddr = NULL;
...
Std_ReturnType Fr_AbortCommunication(uint8 Fr_CtrlIdx) {
    ...
    retval = Fr_Internal_SetCtrlChiCmd(&Fr_Cfg, Fr_CtrlIdx, (uint8)CHI_CMD_FREEZE);
...
Std_ReturnType Fr_HaltCommunication(uint8 Fr_CtrlIdx) {
    ...
    retval = Fr_Internal_SetCtrlChiCmd(&Fr_Cfg, Fr_CtrlIdx, (uint8)CHI_CMD_HALT);
...
Std_ReturnType Fr_AllowColdstart(uint8 Fr_CtrlIdx) {
    ...
    /* @req FR180 */
    VALIDATE_W_RV( !((Fr_Cfg.Fr_CCPocState[Fr_CtrlIdx] == FR_POCSTATE_DEFAULT_CONFIG) ||
                    (Fr_Cfg.Fr_CCPocState[Fr_CtrlIdx] == FR_POCSTATE_CONFIG) ||
                    (Fr_Cfg.Fr_CCPocState[Fr_CtrlIdx] == FR_POCSTATE_HALT)),
                    FR_ALLOWCOLDSTART_ID, FR_E_INV_POCSTATE, E_NOT_OK );

    retval = Fr_Internal_SetCtrlChiCmd(&Fr_Cfg, Fr_CtrlIdx, (uint8)CHI_CMD_ALLOW_COLDSTART);
...
Std_ReturnType Fr_StartCommunication(uint8 Fr_CtrlIdx) {
    ...
    /* @req FR175 */
    VALIDATE_W_RV( (Fr_Cfg.Fr_CCPocState[Fr_CtrlIdx] == FR_POCSTATE_READY),
                    FR_STARTCOMMUNICATION_ID, FR_E_INV_POCSTATE, E_NOT_OK );

    /* @req FR177 */
    /* @req FR176 */
    retval = Fr_Internal_SetCtrlChiCmd(&Fr_Cfg, Fr_CtrlIdx, (uint8)CHI_CMD_RUN);
...
Std_ReturnType Fr_SendWUP(uint8 Fr_CtrlIdx) {
    ...
    retval = Fr_Internal_SetCtrlChiCmd(&Fr_Cfg, Fr_CtrlIdx, (uint8)CHI_CMD_WAKEUP);
...
Std_ReturnType Fr_TransmitTxLPdu(uint8 Fr_CtrlIdx, uint16 Fr_LPduIdx, const uint8* Fr_LSduPtr, uint8 Fr_LSduLength) {
    ...
    VALIDATE_W_RV( (trigConfPtr->FrMsgBufferCfg[trigIdx].FrDataPartitionAddr != NULL), \
                                ...
                                uint8* Fr_LSduLengthPtr )
{
    ...
    *Fr_LPduStatusPtr = FR_NOT_RECEIVED;
    ...
    /* @req FR237 */
    newData = Fr_Internal_CheckNewData(&Fr_Cfg, Fr_CtrlIdx, msgBuffrIdx);

    if (newData == E_OK) {
        retval = Fr_Internal_GetNewData(&Fr_Cfg, Fr_CtrlIdx, trigIdx, Fr_LSduPtr, Fr_LSduLengthPtr);
