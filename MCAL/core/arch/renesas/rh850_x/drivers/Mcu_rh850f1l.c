...
#include "irq.h"
#endif
/*lint -save -e9048 Renesas include file */
...
const core_info_t core_info_list[] = {
    ...
    .name = "RH850G3M",
    .pvr = 0,
    .info = "Max Freq: 80Mhz, ICache: Yes (8KB)",
    },
};
...
static void initPll( const Mcu_ClockSettingConfigType *cPtr ) {
    ...
    /*--  Setup Gain Depending on Crystal ---  */
    ...
    /* --- Setup PLL  */
    PLLC = cPtr->Mcu_Hw_ClockSettings->pll_PLLxC;
    PROTECTED_WRITE(PROTCMD1,PLLE,0x01u);   		/* Enable */
    while((PLLS&0x04u) != 0x04u) {};            	/* Check active */
...
Std_ReturnType Mcu_Hw_InitClock( const Mcu_ClockSettingConfigType *clkPtr ) {
    ...
    /*----  CAN Clock ----*/
    /* Set to  CPUCLK (default) */
    PROTECTED_WRITE(PROTCMD1,CKSC_ICANS_CTL,0x3u);
    while(CKSC_ICANS_ACT!=0x3u) {};

    /* Enable CANOSC and set divider to 1 */
...
static uint32 getCpuClk( Mcu_ClockType clockIdx ) {
    ...
    if( clockSelect == 1uL) {
        ...
        /* C_CPLLCLK */
        clock = Mcu_Global.config->McuClockSettingConfig[clockIdx].Mcu_Hw_ClockSettings->pll_Clk;
    } else {
        clock = 0;
    }
    return clock;
    ...
    switch(type) {
        ...
        case C_AWO_TAUJ:
            ...
            if( CKSC_ATAUJS_ACT == 1u ) {
                ...
                /* C_PPLLCLK2 */
                clock = Mcu_Global.config->McuClockSettingConfig[clockIdx].Mcu_Hw_ClockSettings->pll_Clk / 2;
            }
            /*lint -restore -e9013 */
