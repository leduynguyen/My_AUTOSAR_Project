...
static void normalStandardNm(NetIdType netHandleId);
...
static void rxNmpduProcessFunc(NetIdType netHandleId);
...
static void initFromLimpHome(NetIdType netHandleId){
    ...
    OsekDirectNm_Networks[netHandleId].nmTxCounter = 0;
    OsekDirectNm_Networks[netHandleId].nmRxCounter = 0;
...
static void initFromBusSleep(NetIdType netHandleId){
    ...
    /** @req OSEKNM007 */
    OsekDirectNm_Networks[netHandleId].nmRxCounter = 0x0;
    OsekDirectNm_Networks[netHandleId].nmTxCounter = 0x0;
...
static void initFromNormal(NetIdType netHandleId){
    ...
    /** @req OSEKNM007 */
    OsekDirectNm_Networks[netHandleId].nmRxCounter += (uint8)0x01;
    ...
    if(1 == OsekDirectNm_Networks[netHandleId].networkStatus.nodeStatus.active) {
        ...
        OsekDirectNm_Networks[netHandleId].nmTxCounter += 1;
        OsekDirectNm_Networks[netHandleId].nmTransmitPdu.OpCode.b = ALIVE_MASK;
        sendNmMessage(netHandleId);
    }

    if((OsekDirectNm_Networks[netHandleId].nmRxCounter <= OSEKNM_RX_COUNT_LIMIT) &&
            (OsekDirectNm_Networks[netHandleId].nmTxCounter <=    OSEKNM_TX_COUNT_LIMIT)) {
...
static void normalStandardNm(NetIdType netHandleId) {
    ...
    OsekDirectNm_Networks[netHandleId].nmRxCounter = 0;
    if(1 == OsekDirectNm_Networks[netHandleId].nmpduptr.OpCode.B.limphome) {

        /* add sender to config.limphome */
        addToLimphomeConfig(netHandleId, OsekDirectNm_Networks[netHandleId].nmpduptr.source);
    }else{
        /* add sender to config.present */
        addToPresentConfig(netHandleId, OsekDirectNm_Networks[netHandleId].nmpduptr.source);

        /* determine logical successor */ /** @req OSEKNM010 */
        newDestination =  determineLSnode(OsekDirectNm_Networks[netHandleId].nmpduptr.source,      \
        ...
        if(1 == OsekDirectNm_Networks[netHandleId].nmpduptr.OpCode.B.ring){
            ...
            memcpy(OsekDirectNm_Networks[netHandleId].nmTransmitPdu.ringData, OsekDirectNm_Networks[netHandleId].nmpduptr.ringData, sizeof(RingDataType));

            /* check destination station to me or destination = source */
            if((OsekDirectNm_Networks[netHandleId].nmpduptr.destination == OsekNm_ConfigPtr->osekNmNetwork[netHandleId].osekDirectNmNodeconfParams->nodeId)
                            || (OsekDirectNm_Networks[netHandleId].nmpduptr.destination == OsekDirectNm_Networks[netHandleId].nmpduptr.source)) {
                /* Set timer TTyp */ /** @req OSEKNM009 */
                OsekDirectNm_Networks[netHandleId].timer.osekdirectNmTTyp = (OsekNm_ConfigPtr->osekNmNetwork[netHandleId].osekDirectNmNodeconfParams->osekdirectNmTTyp + 1);
                ...
                isSkippedRing = isNodeSkipped(OsekDirectNm_Networks[netHandleId].nmpduptr.source, OsekNm_ConfigPtr->osekNmNetwork[netHandleId].osekDirectNmNodeconfParams->nodeId, OsekDirectNm_Networks[netHandleId].nmpduptr.destination);
    ...
    if(0 != OsekDirectNm_Networks[netHandleId].timer.osekdirectNmTTyp) {
        ...
        if(1 == OsekDirectNm_Networks[netHandleId].timer.osekdirectNmTTyp) {
            ...
            if(1 == OsekDirectNm_Networks[netHandleId].networkStatus.nodeStatus.active) {
                ...
                sendNmMessage(netHandleId);
                OsekDirectNm_Networks[netHandleId].nmTxCounter++;
                OsekDirectNm_Networks[netHandleId].networkStatus.nodeStatus.ringdataNotallowed = 1;
            }

            if(OsekDirectNm_Networks[netHandleId].nmTxCounter > OSEKNM_TX_COUNT_LIMIT) {
 ...
 * @param netHandleId
 */
static void rxNmpduLimpHomemsg(NetIdType netHandleId) {
    NetIdType netId;
    if((1 == OsekDirectNm_Networks[netHandleId].networkStatus.nodeStatus.bussleep) && (1 == OsekDirectNm_Networks[netHandleId].nmpduptr.OpCode.B.sleepAck)) {
        netId = OsekNm_Internal_GetNetId(netHandleId);
        D_Offline(netId);
        OsekDirectNm_Networks[netHandleId].timer.osekdirectNmTWbs = (OsekNm_ConfigPtr->osekNmNetwork[netHandleId].osekDirectNmNodeconfParams->osekdirectNmTWbs + 1);
...
static void rxNmpduSleepAckProc(NetIdType netHandleId) {
    NetIdType netId;
    if(1 == OsekDirectNm_Networks[netHandleId].nmpduptr.OpCode.B.sleepAck) {
        if(1 == OsekDirectNm_Networks[netHandleId].networkStatus.nodeStatus.bussleep) {
            /* NMInitBusSleep */
            netId = OsekNm_Internal_GetNetId(netHandleId);
            D_Offline(netId);
...
static void rxNmpduProcessFunc(NetIdType netHandleId) {
    ...
    switch(OsekDirectNm_Networks[netHandleId].nmState){
        ...
        case OSEKNM_NORMALPREPSLEEP:
            /* figure 38 in ISO doc */
            normalStandardNm(netHandleId);
            rxNmpduSleepAckProc(netHandleId);
            /* Figure33 - process for NM_stNormalPrepSleep only */
            if(OSEKNM_NORMALPREPSLEEP == OsekDirectNm_Networks[netHandleId].nmState){
                if(0 == OsekDirectNm_Networks[netHandleId].nmpduptr.OpCode.B.sleepInd){
        ...
        case OSEKNM_LIMPHOME: /* Figure 35 - Received msg part */
            if(1 == OsekDirectNm_Networks[netHandleId].networkStatus.nodeStatus.active) {
                if(1 == OsekDirectNm_Networks[netHandleId].nmMerker.B.limphome) { /** @req OSEKNM029 */
                    rxNmpduLimpHomemsg(netHandleId);/** @req OSEKNM027 */ /** @req OSEKNM030 */
                }else {
                    if(1 == OsekDirectNm_Networks[netHandleId].nmpduptr.OpCode.B.sleepAck) {
                        netId = OsekNm_Internal_GetNetId(netHandleId);
                        D_Offline(netId);
                ...
                rxNmpduLimpHomemsg(netHandleId);
            }
            break;
...
void OsekDirectNm_MainFunction(NetIdType netId){
    ...
    if(TRUE == OsekDirectNm_Networks[netHandleId].rcvdNmpdu) {
        ...
        rxNmpduProcessFunc(netHandleId);
    }
...
void OsekDirectNm_RxIndication(NetIdType netId, const OsekNm_PduType* nmPdu) {
    ...
    OsekDirectNm_Networks[netHandleId].networkStatus.nodeStatus.busoff = 0;
    memcpy(&OsekDirectNm_Networks[netHandleId].nmpduptr, nmPdu, sizeof(OsekNm_PduType));
