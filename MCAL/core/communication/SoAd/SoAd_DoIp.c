...
static void discardIpMessage(uint16 socketHandle, uint16 len, uint8 *rxBuffer)
{
    ...
    {
        (void)recv(socketHandle, rxBuffer, SOAD_RX_BUFFER_SIZE, 0);
        len -= SOAD_RX_BUFFER_SIZE;
    }

    if(len > 0){
        (void)recv(socketHandle, rxBuffer, len, 0);
    }
}
...
static void registerSocket(uint16 slotIndex, uint16 sockNr, uint16 activationType, uint16 sa) {
    CONFIG_ASSERT(slotIndex < DOIP_MAX_TESTER_CONNECTIONS);
        ...
        if (lookupResult == LOOKUP_SA_TA_OK) {
            ...
            if (SoAd_BufferGet(SOAD_RX_BUFFER_SIZE, &pduInfo.SduDataPtr)) {
                ...
                if (result == BUFREQ_OK && len > 0) {
                    ...
                    (void)recv(SocketAdminList[sockNr].ConnectionHandle, rxBuffer, 12, 0);
                    (void)recv(SocketAdminList[sockNr].ConnectionHandle, pduInfo.SduDataPtr, diagnosticMessageLength, 0);
...
void DoIp_HandleTcpRx(uint16 sockNr)
{
    ...
    if (SoAd_BufferGet(SOAD_RX_BUFFER_SIZE, &rxBuffer)) {
        nBytes = recv(SocketAdminList[sockNr].ConnectionHandle, rxBuffer, SOAD_RX_BUFFER_SIZE, MSG_PEEK);
                ...
                if ((payloadLength + 8) <= SOAD_RX_BUFFER_SIZE) {
                    if ((payloadLength + 8) <= nBytes) {
                        ...
                        case 0x0001:	// Vehicle Identification Request
                            nBytes = recv(SocketAdminList[sockNr].ConnectionHandle, rxBuffer, payloadLength + 8, 0);
...
void DoIp_HandleUdpRx(uint16 sockNr)
{
    ...
    if (SoAd_BufferGet(SOAD_RX_BUFFER_SIZE, &rxBuffer)) {
        nBytes = recvfrom(SocketAdminList[sockNr].SocketHandle, rxBuffer, SOAD_RX_BUFFER_SIZE, MSG_PEEK, (struct sockaddr*)&fromAddr, &fromAddrLen);
        ...
        if (nBytes >= 8) {
            ...
            if (((rxBuffer[0] == 1) || (rxBuffer[0] == 2)) && (((uint8)(~rxBuffer[1]) == 1) || ((uint8)(~rxBuffer[1]) == 2))) {
                ...
                if ((payloadLength + 8) <= SOAD_RX_BUFFER_SIZE) {
                    if ((payloadLength + 8) <= nBytes) {
                        // Grab the message
                        nBytes = recvfrom(SocketAdminList[sockNr].SocketHandle, rxBuffer, payloadLength + 8, 0, (struct sockaddr*)&fromAddr, &fromAddrLen);
