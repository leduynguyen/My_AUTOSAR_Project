    ...
    for (pduCnt = 0; pduCnt < frameTrigCfgPtr->FrIf_NumberPdusInFrame; pduCnt++) {
...
#if (FRIF_BIG_ENDIAN_USED == STD_ON)
        if (BIG_ENDIAN == frameTrigCfgPtr->FrIf_FrameStructureRef->FrIf_ByteOrder){
        ...
        if (TRUE == resFlag) {
            ...
            if (ret != E_OK) {
                ...
                if (BIG_ENDIAN == frameTrigCfgPtr->FrIf_FrameStructureRef->FrIf_ByteOrder){
                    /*lint -e644 pduLastByteIndex is set in the case of big endian */
...
void FrIf_Internal_HandleReceiveAndStore(uint8 frIfIdx, PduIdType frLPduIdx, uint8 frameCfgIdx)
{
    ...
    if (frameTrigCfgPtr->FrIf_FrameStructureRef->FrIf_ByteOrder == BIG_ENDIAN) {
