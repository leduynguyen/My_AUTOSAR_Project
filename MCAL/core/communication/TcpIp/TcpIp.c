...
#include "Det.h"
#endif
#define TCPIP_DET_REPORTERROR(_api,_err)   (void)Det_ReportError(MODULE_ID_TCPIP, 0, (_api), (_err))
#else
...
static void LwIp_EthernetifInit(struct netif *netif){
    uint8 macAddr[TCPIP_ETH_MAC_ADDR_SIZE] = {0};
    EthIf_GetPhysAddr(TCPIP_ETH_CTRL_INDEX,macAddr);
...
static void LwIp_RxIndication( uint8 CtrlIdx, Eth_FrameType FrameType, boolean IsBroadcast, const uint8* PhysAddrPtr,
    ...
    if (p != NULL){
       ...
       MEMCPY((uint8*)p->payload+(2*TCPIP_ETH_MAC_ADDR_SIZE),&AdaptFrameType,TCPIP_ETH_FRAMETYPE_SIZE);/*lint !e9016,array index is not usable */
...
/**
  ...
  */
static boolean LwIP_AddNetIf( struct ip_addr *ipaddr, struct ip_addr *netmask, struct ip_addr *gw,
...
static void IP4NetmaskToIpAddr(struct ip_addr *NetmaskAddr, uint8 Netmask)
{
    uint8 addr[4];
    ...
    if(sockFd < 0){
        ...
        if(E_OK == TcpIp_GetFirstFreeSocket(&socketId)){
            ...
            *SocketIdPtr = socketId;
            if(ERR_OK != ioctlsocket(sockFd, FIONBIO, &on)){  /*lint !e569 !e970 !e9053 LWIP */ // Set socket to non block mode
                result = E_NOT_OK;
            }
...
Std_ReturnType TcpIp_Close(TcpIp_SocketIdType SocketId, boolean Abort){
    ...
    if(ERR_OK == closesocket(TcpIp_SocketAdmin[SocketId].socketHandle)){
      if(TRUE == TcpIp_SocketAdmin[SocketId].socketProtocolIsTcp){
            event = TCPIP_TCP_CLOSED;
        }else{
            event = TCPIP_UDP_CLOSED;
    ...
    if (E_OK == result) {
        ...
        if(bind(SocketId, (struct sockaddr *)&sLocalAddr, sizeof(sLocalAddr)) < 0) { /*lint !e929 !e740 */ /* pointer to pointer no harm */
            ...
            if(result == E_OK){
                ...
                if(ERR_OK == getsockname(SocketId, (struct sockaddr *)&sLocalAddr, &addrlen)){ /*lint !e929 !e740 */ /* pointer to pointer no harm */
...
Std_ReturnType TcpIp_TcpConnect(TcpIp_SocketIdType SocketId, const TcpIp_SockAddrType* RemoteAddrPtr){
    ...
    conRes = connect(TcpIp_SocketAdmin[SocketId].socketHandle, (struct sockaddr *)&socketAddress, sizeof(struct sockaddr)); /*lint !e929 !e740 */ /* pointer to pointer no harm */
    if(ERR_OK == conRes){
        result = E_OK;
...
Std_ReturnType TcpIp_TcpListen(TcpIp_SocketIdType SocketId, uint16 MaxChannels){
    ...
    if(ERR_OK == listen(TcpIp_SocketAdmin[SocketId].socketHandle, MaxChannels)){ /* BSD style */
...
Std_ReturnType TcpIp_RequestIpAddrAssignment(TcpIp_LocalAddrIdType LocalAddrId, TcpIp_IpAddrAssignmentType Type,
    ...
    struct ip_addr netmask; /*lint !e9046  No Typographical ambiguity */
    struct ip_addr gw;
    Std_ReturnType result;
...
TcpIp_ReturnType TcpIp_GetRemotePhysAddr(uint8 CtrlIdx, const TcpIp_SockAddrType* IpAddrPtr, uint8* PhysAddrPtr,
    ...
    if (etharp_find_addr(netif, &ip, &ethaddr_ret, &ipaddr_ret) > -1)
    {
        ...
        if(TRUE == initRes){
            ...
            if(ERR_OK != etharp_query(netif, &ip, NULL)){
    ...
    if( clientFd != (-1))
    {
        ...
        if(E_OK == TcpIp_GetFirstFreeSocket(&socketIdConn)){
            ...
            if(E_OK == accepted){
                ...
                if(ERR_OK != ioctlsocket(clientFd, FIONBIO, &on)){  /*lint !e970 !e569 !e9053 LWIP */ // Set socket to non block mode
