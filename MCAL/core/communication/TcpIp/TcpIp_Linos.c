...
#define TCPIP_DET_REPORTERROR(_api,_err)                              \
    do {                                                            \
        (void)Det_ReportError(MODULE_ID_TCPIP, 0, _api, _err);    \
    } while(0)
...
static uint16 TcpIp_SendIpMessage(int SocketHandle, const uint8* DataPtr, const TcpIp_SockAddrType* RemoteAddrPtr, uint16 TotalLength)
{
	...
	if (RemoteAddrPtr == NULL) {
		/* TCP */
		bytesSent = send(SocketHandle, (uint8*)DataPtr, TotalLength, 0);
	} else {
		/* UDP */
		...
		bytesSent = sendto(SocketHandle, (uint8*)DataPtr, TotalLength, 0, (struct sockaddr *)&toAddr, sizeof(toAddr));

		TcpIp_IpAddrToChar((char *)&toAddrString, toAddr.sin_addr.s_addr);
		logger_mod ((LOGGER_MOD_TCP|LOGGER_SUB_TCP_SEND), LOG_INFO, "TcpIp_SendIpMessage to %s:%d bytesSent [%d] TotalLength [%d]",
				toAddrString, RemoteAddrPtr->port,
    ...
    if(sockFd < 0){
        ...
        if(E_OK == TcpIp_GetFirstFreeSocket(&socketId)){
            ...
            *SocketIdPtr = socketId;
            ...
            //int on = 1;
            //ioctlsocket(sockFd, FIONBIO, &on);    // Set socket to non block mode
	...
	if(bind(TcpIp_SocketAdmin[SocketId].socketHandle, (struct sockaddr *)&sLocalAddr, sizeof(sLocalAddr)) < 0) {
		...
		if(ptrLocalAddr->AddressType == TCPIP_MULTICAST){
			...
			if(TcpIp_CtrlAdmin[ptrLocalAddr->TcpIpCtrlRef].ptrLocalUnicastAddr != NULL){
				if(TcpIp_CtrlAdmin[ptrLocalAddr->TcpIpCtrlRef].ptrLocalUnicastAddr->StaticIpAddrConfig->ArcValid){
