...
#error PduR: Expected BSW module version to be 2.1.*
#endif
...
void PduR_Init (const PduR_PBConfigType* ConfigPtr) {
    ...
    if (PduRState != PDUR_UNINIT) {
        ...
        /* @req PDUR0709 */
        for( uint16 i = 0; i < PduRConfig->NofRoutingPathGroups; i++ ) {
            PdurSetRoutingPathEnabled(i, PduRConfig->RoutingPathGroups[i].EnabledAtInit);
        }
#endif
        /* @req PDUR326 */
...
PduR_PBConfigIdType PduR_GetConfigurationId (void) {
    ...
    if( PDUR_UNINIT == PduRState ) {
       PDUR_DET_REPORTERROR(MODULE_ID_PDUR, PDUR_INSTANCE_ID, PDUR_SERVICEID_GETCONFIGURATIONID, PDUR_E_INVALID_REQUEST);
...
void PduR_EnableRouting(PduR_RoutingPathGroupIdType id) {
    /* @req PDUR119 */
    if( PDUR_UNINIT == PduRState ) {
        PDUR_DET_REPORTERROR(MODULE_ID_PDUR, PDUR_INSTANCE_ID, PDUR_SERVICEID_ENABLEROUTING, PDUR_E_INVALID_REQUEST);
        /*lint -e{904} Return statement is necessary in case of reporting a DET error */
        return;
    }
    ...
    if( id < PduRConfig->NofRoutingPathGroups ) {
        ...
        /* @req PDUR647 */
        /* @req PDUR648 */
        PDUR_DET_REPORTERROR(MODULE_ID_PDUR, PDUR_INSTANCE_ID, PDUR_SERVICEID_ENABLEROUTING, PDUR_E_ROUTING_PATH_GROUP_ID_INVALID);
    }
}
...
void PduR_DisableRouting(PduR_RoutingPathGroupIdType id) {
    ...
    if( PDUR_UNINIT == PduRState ) {
        PDUR_DET_REPORTERROR(MODULE_ID_PDUR, PDUR_INSTANCE_ID, PDUR_SERVICEID_DISABLEROUTING, PDUR_E_INVALID_REQUEST);
        /*lint -e{904} Return statement is necessary in case of reporting a DET error */
        return;
    }
    ...
    if(id < PduRConfig->NofRoutingPathGroups) {
        ...
        /* @req PDUR716 */
        /* @req PDUR649 */
        PDUR_DET_REPORTERROR(MODULE_ID_PDUR, PDUR_INSTANCE_ID, PDUR_SERVICEID_DISABLEROUTING, PDUR_E_ROUTING_PATH_GROUP_ID_INVALID);
    }
}
