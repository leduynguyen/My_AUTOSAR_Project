...
Std_ReturnType VerifyAllEraBitsZero(void) {
    ...
    for (uint8 chanIndex = 0; ((chanIndex < CANNM_CHANNEL_COUNT) && (ret == E_OK)); chanIndex++) {
        for (uint8 eraByteindex = 0; eraByteindex < CANNM_PNC_INFO_LEN; eraByteindex++) {
...
void restartTimerForInternalExternalRequests(CanNm_Internal_RAType *calcEIRA);
/**
...
void restartTimerForInternalExternalRequests(CanNm_Internal_RAType *calcEIRA) {
    ...
    for (byteNo = 0; byteNo < CANNM_PNC_INFO_LEN; byteNo++) {
...
static inline Std_ReturnType CanNm_Internal_RxProcess(const CanNm_ChannelType* ChannelConf, CanNm_Internal_ChannelType* ChannelInternal, boolean pni ) {
    ...
    if (!isUpdated) {
        ...
        for (i = 0; i < CANNM_PNC_INFO_LEN; i++) {
...
#endif /* (CANNM_PDU_RX_INDICATION_ENABLED == STD_ON) */
