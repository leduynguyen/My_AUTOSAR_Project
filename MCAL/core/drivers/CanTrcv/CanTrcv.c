...
Std_ReturnType CanTrcv_SetOpMode(uint8 Transceiver, CanTrcv_TrcvModeType OpMode)
{
    ...
    trcv = CanTrcv_ConfigPtr->CanTrcvChnlIdToTrcvCntMap[Transceiver] ;

    /* @req SWS_CanTrcv_00123 */
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_SET_OPMODE_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
    currMode = CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_TrcvCurMode;
    ...
    if (FALSE == CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvEnableStatus) {
        ret = E_NOT_OK;
    }
    else {
        spiSeq = CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvSpiSequenceConfig;
        ...
        if (TRUE == CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvWakeupByBus)
        {
            ...
            ret = initializeSelectiveWakeup(trcv,&wakeUpReason,CANTRCV_SET_OPMODE_ID);
        }
#else
        (void)wakeUpReason;
#endif
        if (E_OK == ret){
            if (E_OK == canTrcvSetMode(trcv,currMode,spiSeq,OpMode))
            {
                CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_TrcvCurMode = OpMode;
#if (CANTRCV_WAKE_UP_SUPPORTED == STD_ON)
                /* We check if wake up & partial networking is supported before we indicate PN availability */
                if ((CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_TrcvCurMode == CANTRCV_TRCVMODE_NORMAL)
                        || (TRUE == CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvPnSupport)
                        || (TRUE == CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvWakeupByBus))
                {
...
Std_ReturnType CanTrcv_GetOpMode(uint8 Transceiver, CanTrcv_TrcvModeType* OpMode)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_GET_OPMODE_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
...
Std_ReturnType CanTrcv_GetBusWuReason(uint8 Transceiver, CanTrcv_TrcvWakeupReasonType* reason)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_GET_BUSWUREASON_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
...
#if (CANTRCV_WAKE_UP_SUPPORTED == STD_ON)
    if ((FALSE == CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvEnableStatus)
            || ((FALSE == CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_CurWakeupFlag))){
        ret =  E_NOT_OK; /* If there is no wake up detected return E_NOT_OK */
    }  else {
        *reason = CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_TrcvWakeupReason ;
    }
#else
    ret = E_NOT_OK;
#endif
...
Std_ReturnType CanTrcv_SetWakeupMode(uint8 Transceiver, CanTrcv_TrcvWakeupModeType TrcvWakeupMode)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_SET_WAKEUPMODE_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
    ...
    if (FALSE == CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvEnableStatus) {
    ...
    } else if (TrcvWakeupMode != CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_CurWakeupMode) {
       ...
       else if (TrcvWakeupMode == CANTRCV_WUMODE_DISABLE)
       {
           ...
           CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_CurWakeupMode = CANTRCV_WUMODE_DISABLE;
           ...
           CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_CurWakeupFlag = FALSE;
           CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_CurWakeupMode = CANTRCV_WUMODE_CLEAR;
       }
...
Std_ReturnType CanTrcv_GetTrcvSystemData(uint8 Transceiver,uint32* TrcvSysData)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_GET_TRCVSYSTEMDATA_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
...
Std_ReturnType CanTrcv_ClearTrcvWufFlag(uint8 Transceiver)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_CLEAR_TRCVWUFFLAG_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);

#if (CANTRCV_WAKE_UP_SUPPORTED == STD_ON)
    const CanTrcv_SpiSequenceConfigType *spiSeq;

    if ((FALSE == CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvEnableStatus)|| (FALSE == CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvPnSupport)) {
        ret = E_NOT_OK;
    } else {
...
Std_ReturnType CanTrcv_ReadTrcvTimeoutFlag(uint8 Transceiver, CanTrcv_TrcvFlagStateType* FlagState)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_READ_TRCVTIMEOUTFLAG_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
...
Std_ReturnType CanTrcv_ClearTrcvTimeoutFlag(uint8 Transceiver)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_CLEAR_TRCVTIMEOUTFLAG_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
...
Std_ReturnType CanTrcv_ReadTrcvSilenceFlag(uint8 Transceiver, CanTrcv_TrcvFlagStateType* FlagState)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_READ_TRCVSILENCEFLAG_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
...
Std_ReturnType CanTrcv_CheckWakeup(uint8 Transceiver)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_CHECK_WAKEUP_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
    ...
    if (FALSE == CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvEnableStatus) {
        ...
        if (wakeupFlag){
            ...
            VALIDATE((E_OK == canTrcvSetMode(trcv,CANTRCV_TRCVMODE_SLEEP,spiSeq, CANTRCV_TRCVMODE_NORMAL)), CANTRCV_CHECK_WAKEUP_ID, CANTRCV_E_NO_TRCV_CONTROL, E_NOT_OK);
            CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_TrcvCurMode = CANTRCV_TRCVMODE_NORMAL;

            CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_CurWakeupFlag = TRUE;
            CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_TrcvWakeupReason = reason;

            /* The following requirements are derived from EcuM fixed sequence dig 9.2.3 in ASR 4.0.3 */
            if (CANTRCV_WUMODE_ENABLE == CanTrcv_Internal.CanTrcv_RuntimeData[trcv].CanTrcv_CurWakeupMode)
            {
                if ((CANTRCV_WU_BY_BUS == reason) && (CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvWakeupSourceRef != CANTRCV_INVALID_WAKEUP_SOURCE))
                {
                    EcuM_SetWakeupEvent(CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvWakeupSourceRef);
                }
...
Std_ReturnType CanTrcv_CheckWakeFlag(uint8 Transceiver)
{
    ...
    VALIDATE((trcv < CANTRCV_CHANNEL_COUNT), CANTRCV_CHECK_WAKEFLAG_ID, CANTRCV_E_INVALID_TRANSCEIVER, E_NOT_OK);
    ...
    for(uint8 trcv = 0; trcv < CANTRCV_CHANNEL_COUNT; trcv++)
    {
                ...
                && (CanTrcv_ConfigPtr->CanTrcvChannel[trcv].CanTrcvEnableStatus))
        {
            ...
            if ((CANTRCV_TRCVMODE_STANDBY == mode) || (CANTRCV_TRCVMODE_SLEEP == mode) ||
                ...
                /* The following requirements are derived from EcuM fixed sequence dig 9.2.3 in ASR 4.0.3 */
                CanTrcv_Hw_ReadWakeupEventStatus(&reason,&wakeupFlag,spiSeq);
