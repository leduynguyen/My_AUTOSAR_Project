...
StatusType TerminateTask(){
        ...
        if ( ThreadTasks[CurrentTaskId].pthread_status != GNULINUX_PTHREAD_FUNCTION_STARTED )
        {
        ...
        /* Instead of exiting a thread, we use a wrapper and reuse the thread. */
        ThreadTasks[CurrentTaskId].pthread_status = GNULINUX_PTHREAD_FUNCTION_TERMINATED;
        longjmp(ThreadTasks[CurrentTaskId].stack_env,1); /* Jump to TaskWrapper setjmp */
        // pthread_exit(E_OK);  /* The thread ends after this */
        return E_OK;
#if 0
        int s;
        void *res;
        /* Code for killing another peer thread */

                logger(LOG_INFO, "TerminateTask OTHER called for %s Id (%d).", ThreadTasks[CurrentTaskId].name, CurrentTaskId);
                // Ask the task/thread to terminate.
                s = pthread_cancel(ThreadTasks[CurrentTaskId].tid);
                if (s != 0) {
                        logger(LOG_ERR, "%s [%d]", "pthread_cancel", s);
                        return E_NOT_OK;
                } else {
                        logger(LOG_INFO, "Task %s Id (%d) cancelled.", ThreadTasks[CurrentTaskId].name, CurrentTaskId);
                }


                /* Join with thread to see what its exit status was */
                s = pthread_join(ThreadTasks[CurrentTaskId].tid, &res);
                if (s != 0) {
                        logger(LOG_ERR, "%s [%d]", "pthread_join", s);
                        return E_NOT_OK;
                }else {
            logger(LOG_INFO, "Task %s Id (%d) exited ok.", ThreadTasks[CurrentTaskId].name, CurrentTaskId);
        }

                if (res == PTHREAD_CANCELED) {
                        logger(LOG_INFO, "Task/thread was canceled");
                } else {
                        logger(LOG_ERR, "Task/thread wasn't canceled (shouldn't happen!)");
                }
#endif
