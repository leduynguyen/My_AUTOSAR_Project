...
/*lint -esym(9003, crc8_tab) MISRA Exception: Readability (MISRA 2012 Rule 8.9, advisory) */
...
/*lint -esym(9003, crc8H2F_tab) MISRA Exception: Readability (MISRA 2012 Rule 8.9, advisory) */
...
uint8 Crc_CalculateCRC8H2F(const uint8* Crc_DataPtr, uint32 Crc_Length, uint8 Crc_StartValue8, boolean Crc_IsFirstCall )
{
    ...
    if (Crc_DataPtr != NULL) {
        crc = (TRUE == Crc_IsFirstCall) ? CRC8_H2F_START_VALUE : (Crc_StartValue8 ^ CRC8_H2F_XOR);
