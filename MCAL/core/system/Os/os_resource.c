...
StatusType GetResource( ResourceType ResID ) {
    ...
    if( OS_SYS_PTR->intNestCnt != 0 ) {
        ...
        if( ((isrPtr->constPtr->resourceMask & (1U<< ResID)) == 0) ||
            ( ResID == RES_SCHEDULER ) || (rPtr->owner != NO_TASK_OWNER) ) {
            rv = E_OS_ACCESS;
            Irq_Restore(flags);
            goto err;
        }
        ...
        if( ResID == RES_SCHEDULER ) {
             ...
             * 3. We don't have high enough prio to get the resource
             */
            rPtr = Os_ResourceGet(ResID);

            if( ((taskPtr->constPtr->resourceAccess & (1U<< ResID)) == 0) ||	/* 1 */
...
StatusType ReleaseResource( ResourceType ResID) {
    ...
    if( ResID == RES_SCHEDULER ) {
        ...
        /* Check we can access it */
        if( (pcbPtr->constPtr->resourceAccess & (1U<< ResID)) == 0 ) {
            rv = E_OS_ID;
            goto err;
        }
        rPtr = Os_ResourceGet(ResID);
...
void Os_ResourceInit( void ) {
     ...
     * MISRA 2004 14.1 (req), MISRA 2012 2.1 (req) unreachable code (if OS_RESOURCE_CNT == 0)
     * MISRA 2004 13.7 (req), MISRA 2012 2.1 (req) condition always true (if OS_RESOURCE_CNT == 0)
     */
#if (OS_RESOURCE_CNT > 0)
    for( unsigned int i=0; i < OS_RESOURCE_CNT; i++) {
        ...
        for( TaskType pi = 0; pi < OS_TASK_CNT; pi++) {
            ...
            if(pcb_p->constPtr->resourceAccess & (1U<<i) ) {
