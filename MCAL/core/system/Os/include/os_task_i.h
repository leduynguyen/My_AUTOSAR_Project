...
#define APPLICATION_CHECK_ACCESS(_mask) \
    { \
...
typedef uint32_t OsPriorityType;
...
typedef struct OsTaskVar {
    ...
    uint8_t 		activations;		/* The number of queued activation of a task */
    ...
    uint32_t	regs[16]; 					/* NOTE: Arch specific regs .. make space for them later...*/
...
typedef struct OsTaskConst {
    ...
    struct OsTimingProtection	*timing_protection;
