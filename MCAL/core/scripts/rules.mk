...
copydist : FORCE
	...
	@mkdir -p $(BDIR)/$(DISTRONAME)/arch
	@mkdir -p $(BDIR)/$(DISTRONAME)/arch/$(ARCH_FAM) 
	@mkdir -p $(BDIR)/$(DISTRONAME)/arch/$(ARCH_FAM)/$(ARCH)
	@mkdir -p $(BDIR)/$(DISTRONAME)/arch/$(ARCH_FAM)/$(ARCH)/kernel
	@cp -R $(ROOTDIR)/arch/$(ARCH_FAM)/$(ARCH)/scripts $(BDIR)/$(DISTRONAME)/arch/$(ARCH_FAM)/$(ARCH)/
	@cp -R $(ROOTDIR)/arch/$(ARCH_FAM)/$(ARCH)/kernel/*.h $(BDIR)/$(DISTRONAME)/arch/$(ARCH_FAM)/$(ARCH)/kernel
	@cp $(ROOTDIR)/system/Os/include/Os.h $(BDIR)/$(DISTRONAME)/include
...
$(build-exe-y): $(dep-y) $(obj-y) $(os-orti-y) $(sim-y) $(libitem-y)
	...
	$(Q)$(CC) -Xlinker -Map=$(basename $(@F)).map -o $@ $(libpath-y) $(obj-y) $(lib-y) $(libitem-y) $(LDFLAGS)
	$(do-ld-post)
	@echo
	@echo "  >>>>>>>  DONE  <<<<<<<<<"
	@echo
...
pb-pc-files = $(addprefix $(pb-pc-prefix)/,$(pb-pc-file-y))

PreCompiledDataHash.h: $(pb-pc-files)
	cat $(pb-pc-files) | md5sum | $(SED) 's/\([a-f0-9]\{16\}\)\([a-f0-9]\{16\}\).*/#define PRE_COMPILED_DATA_HASH_LOW 0x\1=#define PRE_COMPILED_DATA_HASH_HIGH 0x\2/' | tr "=" "\n" > $@
